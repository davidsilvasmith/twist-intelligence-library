...
def render_page(title, body, description, back='/index.html'):
...
for mc in (root/'wiki/masterclasses').glob('*.md'):
    ...
    (root/'episodes'/f'{ep.lower()}.html').write_text(render_page(f'Masterclass: {ep} - {title} | TWiST Wiki', body, f'{ep} masterclass page with linked founders, entities, and audited TWiST context.'))
...
for founder in data['trajectory_delta']:
    ...
    if name == "Andrew D'Souza":
        vc_jump = '<a class="inline-block mt-4 text-green-700 underline" href="/index.html">Jump to Creandum on the Venture Constellation</a>'
    elif name == 'Victor Riparbelli':
        vc_jump = '<a class="inline-block mt-4 text-green-700 underline" href="/index.html">Jump to GV and NEA on the Venture Constellation</a>'
    elif name == 'Rob May':
        vc_jump = '<a class="inline-block mt-4 text-green-700 underline" href="/index.html">Jump to HalfCourt Ventures on the Venture Constellation</a>'
    elif name == 'Ryan Carson':
        vc_jump = '<a class="inline-block mt-4 text-green-700 underline" href="/index.html">Jump to the OpenClaw ecosystem node on the Venture Constellation</a>'
    ...
    (root/'founders'/f'{slug(name)}.html').write_text(render_page(f'Founder Trajectory: {name} | TWiST Wiki', body, f'{name} trajectory page with audited company context and TWiST appearance history.'))
